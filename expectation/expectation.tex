...
You conclude that
\begin{multipleChoice}
\choice[correct]{There is no harm in guessing.}
\choice{You shouldn't guess.}
\choice{We can't conclude anything.}
\end{multipleChoice}
\end{exercise}

\begin{exercise}
Marcus and Dave play following game.
Marcus randomly selects card from a standard deck.
If he selects $\clubsuit$ then Dave gives Marcus $\$4$.
Otherwise Marcus gives Dave $\$2$.
The expected proceeds for Marcus are
$\answer{-1/2}$.
  \begin{exercise}
  The expected proceeds for Dave are
  $\answer{1/2}$.
  \end{exercise}
  \begin{exercise}
  You conclude that the game is unfair for
  \begin{multipleChoice}
  \choice[correct]{Marcus}
  \choice{Dave}
  \end{multipleChoice}
  \end{exercise}
\end{exercise}
...
\begin{exercise}
At charity event can purchase one of $100$ tickets for~$\$2$
The prize is $\$50$.
Calculate expected proceeds of a participant who buys one ticket.
\begin{itemize}
\item The outcome $48=50-2$ occurs with probability $\answer{1/100}$ if the player wins.
\item The outcome $-2$ occurs with probability $\answer{99/100}$ if the player loses.
\item So the expected proceeds are $\$\answer{-3/2}$.
\end{itemize}
\end{exercise}
...
\begin{image}
\includegraphics{LetsMakeADeal}
\end{image}

We could put another exercise here.
We could put still another exercise here.
\end{document}
http://497a6980.ngrok.com/course/marcusjbishop/elementaryProbability/master/expectation/expectation.tex
